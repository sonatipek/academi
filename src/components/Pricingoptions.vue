<template>
    <div class="border border-primary-subtle rounded-4 w-50 d-flex flex-column justify-content-between">
        ...
        <ul class="mt-3">
            <li v-for="item in items" :key="item.id">{{ item.feature }}</li>
        </ul>

        <div class="fw-bold fs-2 text-center">
            {{ ucret }}₺ <span class="fs-6 text-secondary fw-light">/{{ loop }}</span>
        </div>

        <a href="#" class="btn btn-primary w-75 align-self-center mt-3 mb-3">Planı Seç</a>
    </div>
</template>

<script>
    export default{
        props: ['items', 'title', 'imgSrc', 'ucret', 'loop']
